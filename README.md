...
Standalone single-topic labeled selector. Connect a Scene Muxer, get a live labeled dropdown, output the selected STRING. No Prompt Director needed — works anywhere a STRING is accepted. Selection survives workflow save/reload.
...
```
Unconnected slots are silently skipped. Template is editable — reorder or weight slots freely.
...
**Normalization:** Every muxed segment ends with exactly two newlines. Subject box is untouched except for ensuring two trailing newlines. Final output collapses all newline runs to exactly two.
...
## License
GPL v3 — free to use and modify, but derivatives must remain open source.
